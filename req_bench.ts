...
Deno.bench("deso", { group: "timing", baseline: true }, async () => {
  const app = new Deso();

  app.get("/user", (context) => context.text("User"));

  app.get("/user/comments", (context) => context.text("User Comments"));

  app.get("/user/avatar", (context) => context.text("User Avatar"));

  app.get("/user/lookup/email/:address", (context) => {
    const address = context.param<string>("address") ?? "";
    return context.text(address);
  });

  app.post("/status", (context) => context.text("Status"));
...
Deno.bench("cheetah", { group: "timing" }, async () => {
  ...
  app.get("/user/lookup/username/:username", (c) => ({
    message: `Hello ${c.req.param("username")}`,
  }));
...
Deno.bench("itty-router", { group: "timing" }, async () => {
  ...
  router.get("/user/lookup/username/:username", (req) => {
    return new Response(
      JSON.stringify({ message: `Hello ${req.params.username}` }),
      {
        headers: {
...
Deno.bench("oak", { group: "timing" }, async () => {
  ...
  router.get("/user/lookup/username/:username", (c) => {
    c.response.body = {
      message: `Hello ${
        Object.fromEntries(c.request.url.searchParams).username
      }`,
    };
  });
